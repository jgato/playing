...
def post_comment_like():
	...
	if "num_likes" in comment:
	     print "exists"
	     counter = int(comment["num_likes"]) + 1
	     print "new value " ,  counter
	else:
             print "first like"
	     counter = 1
	
	print "updating body " + comment['body']
	last_error = posts.update( {"permalink": permalink, "comments.author": comment['author'] }, { "$set" : { "comments.$.num_likes" : counter } })
	print last_error
